...
const BASE_LABELS: Record<BaseRef, string> = {
  sol: "Surface sol", murs: "Surface murs",
  volume: "Volume", perimetre: "Périmètre", manuel: "Manuel",
};

const USAGES = ["Mur", "Sol", "Plafond"];

const EMPTY_FORM = (pieceId: number): PostePayload => ({
  piece_id: pieceId,
  corps_metier: "",
...
export default function PieceDetail() {
  ...
  const load = async () => {
    const [p, ps, mats, sug, inc, deps] = await Promise.all([
  ...
  // Dépendances filtrées pour le matériau sélectionné + usage
  const matDeps = selectedMat
    ? allDeps.filter((d) => {
        if (d.materiau_principal_id !== selectedMat.id) return false;
        if (!selectedUsage) return true;
        return !d.usage || d.usage === selectedUsage;
      })
    : [];

  const close = () => {
    setShowForm(false); setError("");
    setSelectedMat(null); setBaseRef("manuel");
    setAcSearch(""); setAcOpen(false);
    setSelectedUsage(""); setExcludedDepIds(new Set());
  };

  const openForm = () => {
    setForm(EMPTY_FORM(pieceId));
    setSelectedMat(null); setBaseRef("manuel");
    setAcSearch(""); setAcOpen(false);
    setSelectedUsage(""); setExcludedDepIds(new Set());
    setShowForm(true);
  ...
  const handleSelectMateriau = (matId: number) => {
    ...
    if (!m) return;
    setSelectedMat(m);
    setExcludedDepIds(new Set()); // reset exclusions
    const base = getBaseRef(m.unite);
    setBaseRef(base);
    const qte = base !== "manuel" ? computeQuantite(base, piece) : 0;
    setForm((f) => ({ ...f, materiau_principal_id: matId, corps_metier: m.corps_metier, quantite_reference: qte }));
  };
  ...
  const save = async () => {
    ...
    const payload: PostePayload = {
      ...
      dependances_exclues: excludedDepIds.size > 0 ? [...excludedDepIds].join(",") : null,
    };
    try {
      await api.createPoste(payload);
      await load();
      close();
  ...
  const matName = (id: number) => materiaux.find((m) => m.id === id)?.nom ?? "—";

  return (
    <div className="p-8 max-w-5xl mx-auto">
      {piece && (
        <>
          <button onClick={() => navigate(`/chantier/${piece.chantier_id}`)}
          ...
          {suggestions.length > 0 && (
            <div className="bg-orange/5 border border-orange/20 rounded-xl p-4 mb-6">
              <p className="text-sm font-semibold text-orange mb-2">💡 Matériaux suggérés</p>
            ...
            <div className="mb-6 space-y-2">
              {alertes.map((a, i) => (
                <div key={i} className={`rounded-xl p-4 flex items-start gap-3 ${
                  a.severite === "bloquant" ? "bg-red-50 border border-red-200" : "bg-yellow-50 border border-yellow-200"
                }`}>
          ...
          {postes.length === 0 ? (
            ...
            <div className="space-y-4">
              {postes.map((poste) => {
                ...
                const marge = (calc as any)?.marge_securite;
                return (
                  <div key={poste.id} className="card">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <p className="font-bold text-bleu">{matName(poste.materiau_principal_id)}</p>
      ...
      {showForm && piece && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="card w-full max-w-lg shadow-xl max-h-[92vh] overflow-y-auto">
            ...
            <div ref={acRef} className="relative mb-4">
              ...
              {acOpen && (() => {
                ...
                const filtered = materiaux.filter(
                  (m) => m.nom.toLowerCase().includes(q) || m.corps_metier.toLowerCase().includes(q)
                );
            ...
            {selectedMat && (
              <div className="mb-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  <button
                    onClick={() => { setSelectedUsage(""); setExcludedDepIds(new Set()); }}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                      !selectedUsage ? "bg-bleu text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                    }`}
                  >
                    Tous usages
                  </button>
                  {USAGES.map((u) => (
                    <button key={u}
                      onClick={() => { setSelectedUsage(u); setExcludedDepIds(new Set()); }}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                        selectedUsage === u ? "bg-orange text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                      }`}
                    >
                      {u}
                    </button>
            ...
            {selectedMat ? (
              <div className="bg-bleu/5 border border-bleu/15 rounded-xl p-4 mb-4">
                <p className="text-xs font-semibold text-bleu uppercase tracking-wide mb-3">Quantité de référence</p>
                {selectedMat.unite === "m²" ? (
                  <>
                    <p className="text-xs text-gray-500 mb-2">Surface à utiliser :</p>
                    <div className="flex gap-2 mb-3 flex-wrap">
                      {(["sol", "murs", "manuel"] as BaseRef[]).map((b) => (
                        <button key={b} onClick={() => handleChangeBase(b)}
                          className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                            baseRef === b ? "bg-orange text-white" : "bg-white border border-gray-300 text-gray-600 hover:border-orange"
                          }`}>
                          {BASE_LABELS[b]}
                          {b !== "manuel" && <span className="ml-1 opacity-70">({computeQuantite(b, piece).toFixed(2)} m²)</span>}
                        </button>
                      ))}
                    </div>
                  </>
                  ...
                  <p className="text-xs text-gray-500 mb-2">
                    Calculé depuis le <span className="font-semibold text-bleu">{BASE_LABELS[baseRef].toLowerCase()}</span>.
                  </p>
                ) : null}
                <div className="flex items-center gap-3">
                  <input
                    ...
                    onChange={(e) => baseRef === "manuel" && setForm({ ...form, quantite_reference: parseFloat(e.target.value) || 0 })}
                  />
                  <span className="text-sm text-gray-500 w-8 shrink-0">{selectedMat.unite}</span>
                </div>
                {baseRef !== "manuel" && selectedMat.unite === "m²" && (
                  <p className="text-xs text-gray-400 mt-2">
                    Verrouillé.{" "}
                    <button className="text-orange underline" onClick={() => handleChangeBase("manuel")}>
                      Saisir manuellement
                    </button>
                  </p>
                )}
              ...
              <div className="mb-4">
                ...
                <input className="input" type="number" step="0.01" min="0"
                  value="" disabled placeholder="Sélectionnez d'abord un matériau" />
              </div>
            )}

            {/* Dépendances à inclure */}
            {selectedMat && matDeps.length > 0 && (
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-4">
                <p className="text-xs font-semibold text-bleu uppercase tracking-wide mb-3">
                  Dépendances à inclure
                  {selectedUsage && <span className="ml-1 text-gray-400 normal-case font-normal">(filtrées pour usage : {selectedUsage})</span>}
                </p>
                <div className="space-y-2">
                  {matDeps.map((dep) => {
                    const depMat = materiaux.find((m) => m.id === dep.materiau_dependant_id);
                    const checked = !excludedDepIds.has(dep.id);
                    return (
                      <label key={dep.id} className="flex items-center gap-3 cursor-pointer group">
                        ...
                        <div className="flex-1 min-w-0">
                          <span className={`text-sm font-medium ${checked ? "text-gray-800" : "text-gray-400 line-through"}`}>
                            {depMat?.nom ?? `Matériau #${dep.materiau_dependant_id}`}
                          </span>
                ...
                {matDeps.every((d) => excludedDepIds.has(d.id)) && (
                  <p className="text-xs text-orange mt-2">⚠ Toutes les dépendances sont décochées.</p>
                )}
              </div>
            )}

            {selectedMat && matDeps.length === 0 && (
              <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 mb-4 text-xs text-gray-400">
                Aucune dépendance définie pour ce matériau
                {selectedUsage ? ` avec l'usage "${selectedUsage}"` : ""}.
              </div>
            )}
            ...
            <div className="mb-4">
              ...
              <textarea className="input h-16 resize-none"
                ...
                placeholder="Lot acheté en entier, chute prévue…" />
            </div>
